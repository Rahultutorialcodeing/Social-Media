...
export default function InternetStatus({ children }: Readonly<{ children: React.ReactNode; }>) {
    ...
    useEffect(() => {
        Disaptch(boderStetus(isloder))
    }, [isloder, InternetStatus])
    ...
    const interCheckHandler = () => {
        setloder(true)
        if (isOnline) {
            toast({
                title: "You are online right now.",
                description: "Now you can access our website.",
                className: "success"
            })
        }
        else {
            toast({
                variant: "destructive",
                title: "Uh oh! You are offline right now.",
                description: "Please connect your internet.",
